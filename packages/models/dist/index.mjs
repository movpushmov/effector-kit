import { createEffect, createEvent, createNode, createStore, is, launch, sample, step, withRegion } from "effector";
...
function exportModelApi(model, getPredicates) {
	...
	for (const key in model["~api"]) {
		...
		if (is.store(element) || is.event(element)) {
			...
			if (is.targetable(element)) Object.defineProperty(unitElement, "target", { value: (map) => {
				const target = createEvent();
				const actionFx = createEffect(async (payload) => {
					...
					const storeRootId = is.store(element) ? element.graphite.meta.rootStateRefId : null;
...
function model({ contract, fn, instances }) {
	...
	const $instances = instances ?? createStore({}, { sid: `$instances/${sid}` });
...
//#endregion
export { child, contract, define, model, ref };
